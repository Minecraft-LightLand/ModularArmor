...
public class RecipeGen {
	...
	private static ResourceLocation getID(Enchantment item) {
		return new ResourceLocation(ModularArmor.MODID, currentFolder + ForgeRegistries.ENCHANTMENTS.getKey(item).getPath());
	}

	@SuppressWarnings("ConstantConditions")
	private static ResourceLocation getID(Enchantment item, String suffix) {
		return new ResourceLocation(ModularArmor.MODID, currentFolder + ForgeRegistries.ENCHANTMENTS.getKey(item).getPath() + suffix);
	}

	@SuppressWarnings("ConstantConditions")
	public static ResourceLocation getID(ItemLike item) {
		return new ResourceLocation(ModularArmor.MODID, currentFolder + ForgeRegistries.ITEMS.getKey(item.asItem()).getPath());
	}

	@SuppressWarnings("ConstantConditions")
	private static ResourceLocation getID(Item item, String suffix) {
		return new ResourceLocation(ModularArmor.MODID, currentFolder + ForgeRegistries.ITEMS.getKey(item).getPath() + suffix);
	}
